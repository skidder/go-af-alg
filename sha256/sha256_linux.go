...
func newAfalg() (a *afalg, err error) {
	...
	if C.bind(C.int(a.tfm_fd), (*C.struct_sockaddr)(unsafe.Pointer(&C.sha256_sa)),
		C.socklen_t(unsafe.Sizeof(C.sha256_sa))) != 0 {
		a.Close()
...
func (a *afalg) Sum() (md [Size]byte, err error) {
	...
	if n != Size {
		return md, fmt.Errorf("invalid sha256 length")
	}
	return md, nil
}
